...
| Etsy SEO Studio | Private Cloudflare Workers + D1 app for Etsy Open API listing SEO audits, encrypted OAuth token storage, listing sync, seller-reviewed optimization suggestions, Jewelry Listing Studio draft creation with ring/earring/bracelet/pendant pricing modes, pair-earring TCW certification labels, bracelet base-length inch scaling with D-E-F/VVS-VS length labels, diamond-carat preview, and metal-baseline price deltas, side-carat-aware pricing, default fee/sale modeling, fast SKU-index lookup, quantity-safe full variation inventory sync, normalized variation matching diagnostics, visual price-change previews, readable API diagnostics, verified price-only and tags-only updates, plus a read-only SEO scraper API bridge for competitor tags, reviews, and search counts |

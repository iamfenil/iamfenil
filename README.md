...
| [Etsy SEO Studio](https://github.com/iamfenil/etsy-seo-studio) | Private Cloudflare Workers + D1 app for Etsy Open API listing operations, jewelry draft creation, and verified updates. It supports optional three-axis Moissanite + Lab Diamond, carat/length, and metal buyer variations with separate per-stone pricing profiles. Target-net-profit pricing now counts the full carat ladder for rings, earrings, and pendants, or the full length ladder for bracelets, even when only one higher option is selected. Existing listings have an end-of-preview variation replacement control with server-authoritative row counts, live-inventory and exact-matrix hash binding, legacy row-state preservation, stale-state rejection, and complete Etsy read-back verification, plus settlement-aware pricing, duplicate-aware SKU lookup, translations, and an approval-gated MCP control plane. |

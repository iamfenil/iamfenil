...
| Etsy SEO Studio | Private Cloudflare Workers + D1 app for Etsy Open API listing SEO audits, encrypted OAuth token storage, listing sync, seller-reviewed optimization suggestions, Jewelry Listing Studio draft creation with IGI certification-aware and side-carat-aware pricing, fast SKU-index existing listing lookup with photo and pricing-engine SKU backfill, full variation inventory sync, visual price-change previews, verified price-only and tags-only updates, plus a read-only SEO scraper API bridge for verified competitor tags, reviews, and search counts |
